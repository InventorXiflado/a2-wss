    ...
    var ContentArticleComponent = BaseComponent.extend({
        ...
        events : {
            ...
            "submit form.feedback-form" : "onFeedbackSubmit",
         	"submit form.feedback-form-article" : "onFeedbackArticleSubmit",
            "keyup textarea" : "onFeedbackTyping",
            "keyup .js-feedback-textarea" : "onFeedbackCharactersCount",
			...
			"click .js-send-cu-contact-us" : "showAlternateOptions",
			'click .js-send-cu-email' : 'onContactUsClick'
        },
        ...
        _onArticleRetrieveSuccess : function(article) {
	        ...
	        if(qs && (qs.CB || qs.casebaseId)) {
                ...
                if(!isEnabledGuidedHelp)
                {
                     ...
                     // Displaying error message if guided help is not enabled for portal
		...
		showAlternateOptions : function(e){
			e.preventDefault();
 			$(".js-send-cu-contact-us").hide();
 			$(".js-send-cu-phone").show();
			$(".js-send-cu-email").show();
        ...
        onFeedbackSubmit : function(e) {
            ...
            this.articleModel.sendFeedback({
                ...
                success : _this._onFeedbackReceivedSuccess,
                error : _this._onFeedbackReceivedError
            });
        },
        onFeedbackArticleSubmit : function(e) {
			var _this = this;
            e.preventDefault();

            this.suggestionModel = this.getModel('suggestion');

			if($(e.target).find('textarea').val().length != 0){
				// submit a request to 'suggestion' API
                //console.log(this.app.getPortalId(),this.articleName,this.app.language.getString('FEEDBACK_DESCRIPTION'),$(e.target).find('textarea').val(),3);
				this.suggestionModel.submitSuggestion({
					data : {
						"portalId" : this.app.getPortalId(),
						"article.name" : this.articleName,
						"article.description" : this.app.language.getString('FEEDBACK_DESCRIPTION'),
						"article.content" : $(e.target).find('textarea').val(),
						"article.suggestion_type" : 3
					},
					success : this._onFeedbackArticleSuccess,
					error : this._onFeedbackArticleError,
					context : this
				});
			}else{
				this._onFeedbackArticleSuccess();
			}

        },
		_onFeedbackArticleSuccess : function(){
				$('.js-rate-area').html('<div class="egce-font-medium" >'+this.app.language.getString('FEEDBACK_SENT') +'</div>');
		},
		_onFeedbackArticleError : function(){
				//$('.js-rate-area').html('<div class="egce-font-medium" >'+this.app.language.getString('FEEDBACK_NOT_SENT') +'</div>');
		},
		...
		onFeedbackCharactersCount : function(){
			var _this = this;

			setTimeout(function(){
				try{
					var maxCharacterCount = 250;
					var currentCount = 0;
					currentCount = this.$('form .js-feedback-textarea').val().length;
					var remainderCount = maxCharacterCount - currentCount;
					//alert(this.app.language.getString('L10N_CHARACTER_COUNT').replace('{0}', remainderCount));
					_this.showCharactersLeft(this.app.language.getString('L10N_CHARACTER_COUNT').replace('{0}', remainderCount));

					if(this.$('form .js-feedback-textarea').val().length >= maxCharacterCount){
						this.$('form .js-feedback-textarea').val(this.$('form .js-feedback-textarea').val().substring(0,maxCharacterCount))
						_this.showCharactersLeft(this.app.language.getString('L10N_CHARACTER_COUNT').replace('{0}', 0));
					}
				}catch(error){
					alert(error);
				}
		...
		showCharactersLeft :  function(msg){
			$(".feedback-form-article .feedback-counter").html(msg);
		},
        ...
        onRateArticle : function(e) {
            ...
            console.log(feedbackControlToUse,ratingScore);
			if((feedbackControlToUse == 5 && this.$('#stars').raty('score')==1)||(feedbackControlToUse == 4 && $(e.target).attr('data-value')==0)){

				this.$('.js-rate-area').children().hide();
				this.$('.js-rate-area').append(this.compileTemplate('feedback-form'));
				this.onFeedbackCharactersCount();
			}else{
				$('.js-rate-area').html('<div class="egce-font-medium" >'+this.app.language.getString('THANKS_FOR_THE_RATING') +'</div>');
			}
        ...
        onPhoneClick : function(e) {
			e.preventDefault();
			e.stopPropagation();
			//$(e.target).html("&nbsp;&nbsp;"+ this.app.language.compileString("ARTICLE_PHONE")+":&nbsp;&nbsp;0844 411 6000" );
			$.ajax({
				type : "POST",
				data : {
