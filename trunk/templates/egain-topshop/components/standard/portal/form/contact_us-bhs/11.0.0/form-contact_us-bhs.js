...
function(CoreComponent, helpers) {
    ...
    var FormContactUsComponent = CoreComponent.extend({
        ...
        submitForm : function(e) {
			...
			$(tform).find('input[name="name"]').val($form.find('input[name="first_name"]').val() + " " + $form.find('input[name="last_name"]').val());

			//pre-validation of the field - before we start reading second field
			...
			$(tform).find('input[name="subject"]').val($form.find('#' + valueOfQuestion1).find('select option:selected').text());
            ...
            var escalation_data = {
                ...
                customerName  : ($form.find('input[name="first_name"]').val() + " " + $form.find('input[name="last_name"]').val()),
			...
			var formatted_message_string = "\n-------------------- FORM DATA ----------------------------\n" +
				...
				($.trim(escalation_data.orderNumber)==this.app.language.compileString("CU_ORDER_NUMBER") ? "" : "Order Number: " + escalation_data.orderNumber + "\n") +
				"Please choose a title: " + escalation_data.nameTitle + "\n" +
				"First Name: " + escalation_data.firstName + "\n" +
				"Last Name: " + escalation_data.lastName + "\n" +
				($.trim(escalation_data.houseNumberStreet)=="" ? "" : "House Number and Street: " + escalation_data.houseNumberStreet + "\n") +
				($.trim(escalation_data.address2)=="" ? "" : "Address 2: " + escalation_data.address2 + "\n") +
...
*/
        ...
        onTopQuestionChange : function(e){
			...
			if(selectedId==""){
				...
				$("#support_que div").css("background-color", "#eee");
				$("#support_que div:first").show().children(":first").attr("disabled", "true").css("backgroundColor", "#eeeeee");
				return;
			}
			$("#support_que div").hide();
			$("#support_que div").css("background-color", "#ffffff");
			$("#"+ selectedId).children(":first").removeAttr("disabled").css("background-color", "#ffffff").end().show();
		},
